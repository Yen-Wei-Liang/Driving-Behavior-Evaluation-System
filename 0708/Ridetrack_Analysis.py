...
class RideTrackAnalyzer:
    ...
    def All_model_v1(self, Train_Data, Train_Label, Test_Data, Test_Label, Validation_DataSet, Feature, Save):
    ...
    def Compute_Accuracy_All(self, DataSet, Feature, Frequency, Save):

        column_names=[]
        for feature in Feature:
            DataSet[f'Filter_{feature}'] = self.Filter_Actions(DataSet[feature], Frequency)
            column_names.append(feature)
            column_names.append(f'Filter_{feature}')
        ...
        if Save:
            result_df2.to_csv(f'{date}_Accuracy_{Feature}_Predict_{time}.csv', index=False)
        return result_df2

    # 觀察預測錯誤是錯在哪
    def Calculate_Action_Prediction_Counts(self, DataSet, Feature):
    
        action_elements = []
    
        for action in DataSet['Action'].dropna().unique():
            counts = DataSet[DataSet['Action'] == action][Feature].value_counts().reset_index()
        ...
        return Calculate_Action[['Action', Feature, 'Count']]
    

    def Calculate_Action_Prediction_Counts_v1(self, Test_Label, test_predict, Feature):
        # 計算混淆矩陣
        conf_matrix = confusion_matrix(Test_Label, test_predict)
        ...
        columns=[]
        for num in range(len(np.unique(Test_Label))):
            columns.append(f'Predicted: {np.unique(Test_Label)[num]}')
        ...
        for i in range(len(labels)):
            ...
            result_df.loc[i] = [action, *conf_matrix[i], accuracy]  # 添加一個未命名的列
                    # 將DataFrame以Markdown格式輸出
