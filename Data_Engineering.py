...
class Preprocessing:
    ...
    def KMeans_Cluster(self, DataSet, Feature, k): 
        ...
        # 儲存分群完資料成 CSV 檔案
        DataSet.to_csv('DataSet_Rotation_Init_Normalized_Cluster.csv')
    ...
    def Test_Vomm(self, TestData, frequency):
        ...
        TestData.to_csv(f'TestData_{frequency}_predict.csv')
    ...
    def Validation_Vomm(self, ValidationData, frequency):
        import time

        # 使用測試集驗證
        sequence = ''.join(ValidationData['Action Element'].astype(str))
        ...
        for i in range(len(subsequences)):
            ...
            ValidationData[prediction_variable] = predictions[prediction_variable]

        ValidationData.to_csv(f'ValidationData_{frequency}_predict.csv')
