...
class RideTrackAnalyzer:
    ...
    # 計算分類的準確度與預測並儲存(2個檔案)
    def All_model(self, DataSet, Validation_DataSet, Label):
    ...
    def Test_Vomm(self, TestData, frequency):
        ...
        action_element_list = TestData['Action Element'].values.tolist()
        
        f = 30//frequency  #取樣不同換算
    ...
    # 傳統閥值分類並儲存檔案
    def Tradition_Category(self, DataSet, Quantiles, Feature):
    ...
    # 傳統閥值編碼並儲存檔案
    def Tradition_Encoding(self, DataSet, Feature):
        ...
        # 構建檔名
        filename = f'{date}_Tradition_Encoding_{time}.csv'  # 使用f-string來將日期和時間插入檔名中
    ...
    # 繪製動作元素所組成之動作
    def Plot_Action_Cluter(self, DataSet, Action1, Action2, Feature, Cluster, Test):
        colors = plt.cm.tab10.colors

        DataSet_Action1 = DataSet[DataSet['Action'] == Action1][:Test]
        DataSet_Action2 = DataSet[DataSet['Action'] == Action2][:Test]
        ...
        # 線
        plt.scatter( DataSet_Action1_Length,  DataSet_Action1[Feature], c=[colors[x] for x in 
        DataSet_Action1 ['Action Element']], zorder=2)

        # Action Element 
        plt.plot(DataSet_Action1_Length, DataSet_Action1[Feature][DataSet_Action1['Action'] == 
        Action1], c='LightBlue' , label=Action1, linewidth=10, zorder=1)    

        plt.scatter(DataSet_Action2_Length, DataSet_Action2[Feature], c=[colors[x] for x in 
        DataSet_Action2['Action Element']], zorder=2)
        ...
        # plt.savefig(f'{Feature} {Action1} vs {Action2}（unite）.png', bbox_inches='tight')
    ...
    def Compute_Accuracy_V3(self, DataSet, frequency):

        import pandas as pd
        DataSet['Filter_Predict'] = self.Filter_Actions(DataSet['Predict'], frequency)
        column_names = ['Action' ,'Predict', 'Filter_Predict']
